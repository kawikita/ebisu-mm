...
-- クレジットカード支払方法管理テーブル
CREATE TABLE
    IF NOT EXISTS credit_card_payment_methods (
...
CREATE TABLE
    IF NOT EXISTS credit_card_installment_status (
...
-- クレジットカード分割払い履歴テーブル
CREATE TABLE
    IF NOT EXISTS credit_card_installments (
...
CREATE TABLE
    IF NOT EXISTS credit_card_revolving_status (
        id INTEGER PRIMARY KEY AUTOINCREMENT, -- ステータスのユニークな識別子
...
-- クレジットカードリボ払い履歴テーブル
CREATE TABLE
    IF NOT EXISTS credit_card_revolving_history (
